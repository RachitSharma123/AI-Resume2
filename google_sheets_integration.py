...
def create_job_tracker_sheet(client, sheet_name="Job Applications Tracker"):
    try:
        ...
        headers = [
            "Date Applied", "Company", "Position", "Job URL", "Status",
            "ATS Score", "Contact Person", "Contact Email", "Follow-up Date",
            "Notes", "Resume Version", "Cover Letter"
        ]
    ...
    try:
        if sheet_id:
            try:
                ...
                st.warning("Could not find existing tracker, creating new one...")
...
def get_statistics(applications):
    if not applications:
        return {
            ...
            "offered": 0, "rejected": 0, "response_rate": 0
        }
    ...
    return {
        "total": total, "applied": applied, "interviewing": interviewing,
        "offered": offered, "rejected": rejected, "response_rate": round(response_rate, 1)
    }
