...
def call_ai_tailor_resume(
    ...
    system_prompt = (
        ...
        " - Every job in experience MUST have EXACTLY 5 bullet points — no more, no less\n"
        " - career_objective MUST be long: minimum 5 sentences, 80–120 words, covering skills, experience breadth, and alignment to the role\n"
        ...
        "8. CERTIFICATIONS: list only the certification name. No status words like 'in progress', 'completed', 'pursuing', 'ongoing', 'expected', or any date/status suffix.\n\n"
        "OUTPUT REQUIREMENTS:\n"
...
def call_ai_compress_resume(resume_json: dict, model: str | None = None) -> dict:
    ...
    system_prompt = (
        ...
        "Your task:\n"
        "1. Keep EXACTLY 5 bullet points per job (keep most impactful, add one if fewer)\n"
        "2. Make bullet points more concise (1 line each when possible)\n"
        "3. Keep career objective long — do NOT shorten it\n"
...
def call_ai_improve_from_ats(
    ...
    system_prompt = (
        ...
        "- Every job MUST have EXACTLY 5 bullet points\n"
        "- career_objective must remain long: minimum 5 sentences, 80–120 words\n"
        "- CERTIFICATIONS: name only, no status words (no 'in progress', 'completed', 'pursuing', 'ongoing', dates, etc.)\n\n"
