    ...
    try:
        if sheet_id:
            try:
                ...
                st.error(f"Could not open existing sheet: {str(e)}")
                st.warning("⚠️ Could not find existing tracker.")
        ...
        try:
            ...
            if "quota" in str(create_error).lower() or "403" in str(create_error):
                st.error("❌ Your Google Drive storage is full!")
                st.warning("**Solution:** Create a Google Sheet manually and share it with the service account:")
                st.code("job-tracker-service@job-thing-485602.iam.gserviceaccount.com")
                st.info("""
                **Steps:**
                1. Go to https://sheets.google.com
                2. Create new sheet named "Job Applications Tracker"
                3. Add headers: Date Applied, Company, Position, Job URL, Status, ATS Score, Contact Person, Contact Email, Follow-up Date, Notes, Resume Version, Cover Letter
                4. Click Share → Add the email above → Give Editor access
                5. Copy the Sheet ID from the URL
                6. Paste it in the field above and click Connect
                """)
            raise Exception(f"Error creating tracker: {str(create_error)}")
